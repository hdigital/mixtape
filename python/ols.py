...
tb = pd.DataFrame(
    {"x": np.random.normal(size=10000), "u": np.random.normal(size=10000)}
)
...
(
    ...
    + p.annotate(
        "text", x=-1.5, y=30, color="red", label="Intercept = {}".format(-0.0732608)
    )
    + p.annotate(
        "text", x=1.5, y=-30, color="blue", label="Slope = {}".format(5.685033)
    )
)
