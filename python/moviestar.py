...
star_is_born = pd.DataFrame(
    {"beauty": np.random.normal(size=2500), "talent": np.random.normal(size=2500)}
)
...
# %% -- plots
(star_is_born.plot.scatter(x="talent", y="beauty", title="All people"))

# %%
(
    star_is_born[star_is_born.star == 1].plot.scatter(
        x="talent", y="beauty", title="Stars"
    )
)

# %%
(
    star_is_born[star_is_born.star == 0].plot.scatter(
        x="talent", y="beauty", title="Other people"
    )
)
