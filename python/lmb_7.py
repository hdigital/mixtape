...
lmb_data = lmb_data[lmb_data.demvoteshare.between(0.45, 0.55)]
...
agg_lmb_data["lagdemvoteshare"] = np.arange(0.01, 1.01, 0.01)
...
(
    ...
    + p.stat_smooth(
        p.aes("lagdemvoteshare", "score", group="gg_group"),
        data=lmb_data,
        method="lm",
        formula="y ~ x + I(x**2)",
    )
...
(
    ...
    + p.geom_point(p.aes(x="lagdemvoteshare", y="score"), data=agg_lmb_data)
    + p.stat_smooth(
        p.aes("lagdemvoteshare", "score", group="gg_group"), data=lmb_data, method="lm"
    )
    + p.xlim(0, 1)
    + p.ylim(0, 100)
    + p.geom_vline(xintercept=0.5)
