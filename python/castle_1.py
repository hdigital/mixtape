...
demo = ("emo", "blackm_15_24", "whitem_15_24", "blackm_25_44", "whitem_25_44")
...
dd_formula = "l_homicide ~ {} + {} + {} + post + C(year) + C(sid)".format(
    "+".join(xvar), "+".join(region), "+".join(lintrend)
)

# Fixed effect regression using post as treatment variable
dd_reg = smf.wls(dd_formula, data=castle, weights=castle["popwt"]).fit(
    cov_type="cluster", cov_kwds={"groups": castle["sid"]}
)
dd_reg.summary()
