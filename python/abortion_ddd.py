...
reg = smf.wls(
    """lnr ~ C(repeal)*C(year) + C(younger)*C(repeal) + C(younger)*C(year) + 
C(yr)*C(year) + C(fip)*t + acc + ir + pi + alcohol + crack + poverty + income + ur""",
    data=abortion_filt,
    weights=abortion_filt.totpop.values,
).fit(cov_type="cluster", cov_kwds={"groups": abortion_filt.fip.values}, method="pinv")

abortion_plot = pd.DataFrame(
    {
        "sd": reg.bse["C(yr)[T.1]:C(year)[T.1986.0]":"C(yr)[T.1]:C(year)[T.2000.0]"],
        "mean": reg.params[
            "C(yr)[T.1]:C(year)[T.1986.0]":"C(yr)[T.1]:C(year)[T.2000.0]"
        ],
...
(
    ...
    + p.geom_rect(
        p.aes(xmin=1986, xmax=1991, ymin=-np.inf, ymax=np.inf), fill="cyan", alpha=0.01
    )
    + p.geom_point()
